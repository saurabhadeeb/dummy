class CompaniesController < ApplicationController
 ...
 def get_content_from_response(resp)
   ...
   source = file.read
   File.delete(file)
   document = Readability::Document.new(source, :tags => %w[div p img a ul ol li code],
   ...
   p = Nokogiri::HTML(document.content)
 ...
 def get_og_image(page)
   image = page.css("meta[property='og:image']").first.attributes["content"].value.to_s
   if !image.blank?
     Rails.logger.debug("In CompaniesController-get_image_srcs - Found og:image")
 ...
 def download_slides(page)
   ...
   if !page.css("div[data-index]").blank?
     ...
     dir_name="slides/"+File.dirname(@base_url).split("/").last+"/"+
       File.basename(@base_url).split("?").first
     ...
     page.css("img.slide_image").each_with_index do |slide, index|
       ...
       current_host=request.host
       current_port=request.port
       img_hash[:url]="#{current_scheme}://#{current_host}:#{current_port}/#{dir_name}/#{basename}"
       img_hash[:size]=50000
       @final_img_srcs<<img_hash
     end
   else
